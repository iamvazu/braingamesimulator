#Developed by Vishal Aditya, er.vishaladitya.tech@gmail.com
#Run this program in background
#Dependencies: PyWin32, Mindwave
#EDITS in the code:
#headset = mindwave.Headset('<CHECK_YOUR_COM_PORT_IN_DEVICE_MANAGER>', 'CC0E')
...
headset = mindwave.Headset('COM13', 'CC0E')
time.sleep(2)
...
#Calculating EYE BLINK from raw values...
        initVal = headset.raw_value 
        time.sleep(0.1)
        blink = abs(headset.raw_value - initVal)
        print "Attention:",attention,"Meditation:",meditation,"Blink Strength:",blink
        ...
        '''#HANDBRAKE    
        if(meditation > 65.0): #right_ctrl
            PressKey(0x9D)
        if meditation < 65.0:
            ReleaseKey(0x9D)'''

        #NITRO    
        if(attention > 65.0):    #space
            print "NITRO"
            PressKey(0x39)
        if attention < 65.0:
            ReleaseKey(0x39)
